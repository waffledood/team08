                                          ...
                                          Grid &grid)
{
  ...
  {
    trajectory.emplace_back(pos_begin.x + Dx * time / duration, pos_begin.y + Dy * time / duration);
  }
                                          ...
                                          const double target_dt, const Grid &grid)
{
  ...
  // (2) generate cubic / quintic trajectory
  // done by students
  CubicHermiteSpline spline(raw_path, t);
  std::vector<Position> smooth_path = spline.samplePoints(target_dt);
...
{  // returns true if the entire path is accessible; false otherwise
  ...
  for (int n = 1; n < trajectory.size(); ++n)
  {
    ...
    grid.los.reset(idx_src, idx_tgt); // interpolate a straight line between points; can do away with los if points are
    fine enough. Index idx = idx_src; while (idx.i != idx_tgt.i || idx.j != idx_tgt.j)
    {
